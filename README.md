...
Now the bundle is ready to be tested against the public [test portal](https://demo.spid.gov.it/validator#/metadata-sp-download) using the
SPID provider metadata endpoint generated by Keycloak.  
The URL can be obtained from the management interface: Identity Provider -> SPID -> SPID service provider metadata.
